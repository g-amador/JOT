...
import static jot.gui.camera.Camera.Type.PERSPECTIVE_FOLLOW;
import static jot.physics.Kinematics.translatePolar;
import org.apache.commons.math3.geometry.euclidean.threed.Vector3D;
import static org.apache.commons.math3.geometry.euclidean.threed.Vector3D.ZERO;

/**
 * Class that implements a perspective follow camera.
 *
 * @author G. Amador {@literal &} A. Gomes
 */
public class PerspectiveFollow extends Camera {
     ...
     * {@inheritDoc}
     */
    public PerspectiveFollow(float fov, float aspectRatio, float size, String cameraId) {
        super(fov, aspectRatio, size, cameraId);
        this.type = PERSPECTIVE_FOLLOW;
    }
    ...
    public void update(Vector3D position, Vector3D rotation, float dt) {
        if (this.position.equals(ZERO)) {
            //this.position = new Vector3D(size * -.8f, size * 0.35f, size * -.8f);
            //this.position = new Vector3D(size * -.48f, size * 0.08f, size * -.48f);
            this.position = new Vector3D(this.size * -.22f, this.size * 0.08f, this.size * -.22f);
        }
